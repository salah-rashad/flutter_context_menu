...
- [x] T027 [US2] Create `example/lib/widgets/tools_panel/structure_tab/entry_properties/` — Property editor panel for the currently selected `EntryNode`, split into: `entry_properties.dart` (root, switches on EntryType), `item_fields.dart`, `header_fields.dart`, `divider_fields.dart` (per-type fields), `property_field_widgets.dart` (shared form primitives), `property_type_header.dart` (type label), `properties_empty_state.dart` (no selection). Barrel at `entry_properties.dart`. NOTE: Icon picker, shortcut selector, text color, and trailing widget are added by T060 (US8).
- [x] T028 [US2] Create `example/lib/widgets/tools_panel/structure_tab/entry_tree_editor/` — Full tree editor split into: `entry_tree_editor.dart` (root CRUD/reorder/submenu), `entry_tree_header.dart` (panel header + Add action), `add_entry_dropdown.dart` (type picker dropdown), `entry_item_view.dart` (single row widget), `hoverable_tree_item.dart` (hover wrapper), `entry_empty_state.dart` (empty list state), `tree_node_utils.dart` (path/depth helpers). Barrel at `entry_tree_editor.dart`.
